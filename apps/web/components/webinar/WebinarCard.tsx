...
/* ------------------------------------------------------------------ */
/*  WebinarCard – calm card for webinar listing                         */
/* ------------------------------------------------------------------ */
...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  ended: {
    label: 'Zakonczony',
    className: 'bg-warm-200 text-earth-600',
  },
};
...
export default function WebinarCard({ webinar, signupCount }: WebinarCardProps) {
  ...
  return (
    <article className="card-calm flex flex-col">
      ...
      {/* Title */}
      <h3 className="font-serif text-lg text-earth-800 leading-snug mb-2">
        {webinar.title}
      </h3>

      {/* Description */}
      {webinar.description && (
        <p className="text-sm text-earth-500 leading-relaxed line-clamp-3 mb-4">
          {webinar.description}
        </p>
      )}

      {/* Meta row */}
      <div className="flex flex-wrap items-center gap-3 text-sm text-earth-400 mb-4">
        ...
        {spotsLeft !== null && (
          <span
            className={`flex items-center gap-1.5 ${
              spotsLeft <= 5 ? 'text-rose-500 font-medium' : ''
            }`}
          >
            <svg
              className="w-4 h-4"
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {tags.map((tag: string) => (
            <span
              key={tag}
              className="px-2 py-0.5 rounded-full bg-warm-100 text-earth-500 text-xs"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="mt-auto pt-2">
        {webinar.status === 'upcoming' ? (
          ...
          <span className="inline-flex items-center justify-center w-full px-5 py-2.5 text-sm text-rose-600 font-medium">
            Trwa teraz
          </span>
        )}
      </div>
